        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [dummy,x] = histogram_dist(currentData); %#ok<ASGLU>
                    if length(x) == 1
            ...
            case 2
                ...
                [xHist,yHist] = histogram_dist(currentData,opt.divFactor,0);
            case 99
...
%                 mh = plot(ah,opt.xValues+xOffset,m,'+r','Color',[0 0.7 0.7],'MarkerSize',12);
                mh = plot(ah,[opt.xValues(1)-0.45 opt.xValues(1)+0.45],[m(1) m(1)],'Color','k','LineWidth',1);
                hold on;
                mh = plot(ah,[opt.xValues(2)-0.45 opt.xValues(2)+0.45],[m(2) m(2)],'Color','k','LineWidth',1);
            end
            if any(opt.showMM==[1,3])
%                 mdh = plot(ah,opt.xValues+xOffset,md,'sg','MarkerSize',12);
                mh = plot(ah,[opt.xValues(1)-0.45 opt.xValues(1)+0.45],[md(1) md(1)],'Color','k','LineWidth',1);
                hold on;
                mh = plot(ah,[opt.xValues(2)-0.45 opt.xValues(2)+0.45],[md(2) md(2)],'Color','k','LineWidth',1);
            end
            if opt.showMM == 4
                mh = plot(ah,opt.xValues+xOffset,m,'+r','Color',[0 0.7 0.7],'MarkerSize',12);
            ...
            if opt.showMM == 6
                mdh(1,:) = plot(ah,squeeze(md(:,1,2:3))',repmat(md(:,1,1)',2,1),'color',[0 0.7 0.7],'lineWidth',2);%,'lineStyle','--');
                mdh(2,:) = plot(ah,squeeze(md(:,2,2:3))',repmat(md(:,2,1)',2,1),'color',[0 0.7 0.7],'lineWidth',1);%,'lineStyle','--');
                mdh(3,:) = plot(ah,squeeze(md(:,3,2:3))',repmat(md(:,3,1)',2,1),'color',[0 0.7 0.7],'lineWidth',1);%,'lineStyle','--');
            end
        case 'flipped'
            if any(opt.showMM==[1,2])
