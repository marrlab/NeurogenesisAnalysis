    ...
    if isfield(parameters,'MS')
        ...
        switch options.MS.plot_type
            ...
            case 1
                ...
                h_nconv = plot(parameters.MS.par(i,ind_nconv),exp(parameters.MS.logPost(ind_nconv)-logPost_max),'d','linewidth',max(1,options.MS.lw-1),'color',options.MS.col);
            case 2
                % negative log-likelihood
                ...
                h_nconv = plot(parameters.MS.par(i,ind_nconv),parameters.MS.logPost(ind_nconv),'d','linewidth',max(1,options.MS.lw-1),'color',options.MS.col); hold on;
            otherwise
